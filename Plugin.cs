...
namespace NetLog
{
    ...
    public class Plugin : BasePlugin
    {
        ...
        [HarmonyPatch(
            typeof(SteamPacketManager),
            nameof(SteamPacketManager.Method_Private_Static_Void_SteamNetworkingMessage_t_Int32_0)
        )]
        [HarmonyPrefix]
        public static void onPacketReceived(SteamNetworkingMessage_t __0, int __1)
        {
            ...
            logPacket(
                SteamManager.Instance.prop_CSteamID_0,
                new Packet(contents),
                PacketFlags.Reliable,
                NetworkChannel.ToClient,
                true
            );
        }
        ...
        )
        {
            ...
            string packet_sent_or_received = received
                ? "received"
                : $"sent going {channel.ToString()} {destination} {is_self}";
            ...
            if (
                channel == NetworkChannel.ToClient
                && SteamPacketManager.ClientPacketHandlers.ContainsKey(ID)
            )
            {
                methodstring =
                    $"Client Handler: {SteamPacketManager.ClientPacketHandlers[ID].Method.Name}";
            }
            else if (
                channel == NetworkChannel.ToServer
                && SteamPacketManager.ServerPacketHandlers.ContainsKey(ID)
            )
            {
